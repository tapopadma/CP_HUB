...
// Recursively finds minimum of d'th dimension in KD tree
// The parameter depth is used to determine current axis.
Node *findMin1Rec(Node* root, int d, unsigned depth)
{
	// Base cases
	if (root == NULL)
		return NULL;
	...
	if (cd == d)
	{
		if (root->left == NULL)
			return root;
		return findMin1Rec(root->left, d, depth + 1);
	}

	// If current dimension is different then minimum can be anywhere
	// in this subtree
	return minNode(root,
		findMin1Rec(root->left, d, depth + 1),
		findMin1Rec(root->right, d, depth + 1), d);
}

// A wrapper over findMinRec(). Returns minimum of d'th dimension
Node *findMin1(Node* root, int d)
{
	// Pass current level or depth as 0
	return findMin1Rec(root, d, 0);
}
...
Node *deleteNodeRec(Node *root, int point[], int depth)
{
	...
	if (arePointsSame(root->point, point))
	{
		...
		if (root->right != NULL)
		{
			...
			Node *min = findMin1(root->right, cd);
		...
		{
			Node *min = findMin1(root->left, cd);
			copyPoint(root->point, min->point);
...
void KDTreeBasic::testKDTreeBasic(){
